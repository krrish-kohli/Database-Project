    ...
    def search_by_infix(self, infix: str, n: int, by_rank: bool):
        ...
        if not by_rank:
            try:
                printed = 0
                start_time = time.time()
                # FIXME: Copy-paste your code from Module 2
            ...
            try:
                ...
                for book in self.bookCatalog:
                  if infix.lower() in book.title.lower():
                    ranked_book = Book(book.key, book.title, book.group, -int(book.rank), book.similar)
                    self.booksByRank.add(ranked_book)
              
                printed = 0
                for i in range(self.booksByRank.size()):
                  
                    top_book = self.booksByRank.remove() # FIXME: Replace None with the book at the top of the heap
                  
                    top_book.rank = -top_book.rank # FIXME: Replace 0 with the absolute value of the rank of the top-ranking book
                    print("-" * 25 + f"\nTop Match #{printed+1}:\n{top_book}\n")
                    printed += 1
                    if printed == n:
                        break
